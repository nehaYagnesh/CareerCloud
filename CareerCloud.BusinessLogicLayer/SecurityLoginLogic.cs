    ...
    public class SecurityLoginLogic : BaseLogic<SecurityLoginPoco>
    {
        ...
        protected override void Verify(SecurityLoginPoco[] pocos)
        {
            ...
            foreach (var poco in pocos)
            {
                ...
                {
                    exceptions.Add(new ValidationException(700, $"Password for SecurityLogin of {poco.Id} cannot be null"));
                }
                else if (poco.Password.Length < 10)
                {
                    exceptions.Add(new ValidationException(700, $"Password for SecurityLogin of {poco.Id} must be at least 10 characters."));
                }
                else if (!requiredExtendedPasswordChars.Any(t => poco.Password.Contains(t)))
                {
                    exceptions.Add(new ValidationException(701, $"Password for SecurityLogin of {poco.Id} must contain an extended character of '$', '*', '#', '_' or '@' ."));
                }

                if (string.IsNullOrEmpty(poco.PhoneNumber))
                {
                    exceptions.Add(new ValidationException(702, $"PhoneNumber for SecurityLogin of {poco.Id} is required"));
                }
                else
                {
                    ...
                    {
                        exceptions.Add(new ValidationException(703, $"PhoneNumber for SecurityLogin of {poco.Id} is not in the required format."));
                    }
                    else
                    {
